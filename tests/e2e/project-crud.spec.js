...
test.describe('Project CRUD Operations', () => {
  ...
  test.describe('Create Project', () => {
    ...
    test('should prevent creating project with duplicate name', async ({ page, request }) => {
      ...
      const hasError = await page.locator('text=/already exists|duplicate/i').isVisible().catch(() => false);
    ...
    test('should handle very long project descriptions', async ({ page }) => {
      ...
      const hasError = await page.locator('text=/too long|maximum|limit/i').isVisible().catch(() => false);
      const hasSuccess = page.url().includes('project');
  ...
  test.describe('Read Project', () => {
    test('should display project list', async ({ page }) => {
      ...
      const projectList = page.locator('[data-testid="project-list"], .project-card, .project-item');
  ...
  test.describe('Update Project', () => {
    ...
    test('should prevent non-owner from editing project', async ({ page, request, browser }) => {
      ...
      const hasError = await page2.locator('text=/unauthorized|forbidden|not allowed/i').isVisible().catch(() => false);
  ...
  test.describe('Project Collaborators', () => {
    test('should add collaborator to project', async ({ page, request }) => {
      ...
      const addCollabButton = page.locator('button:has-text("Add Collaborator"), button:has-text("Invite")');
    ...
    test('should remove collaborator from project', async ({ page, request }) => {
      ...
      // Add collaborator
      await request.post(`${API_URL}/projects/${project._id}/collaborators`, {
        headers: { 'Authorization': `Bearer ${accessToken}` },
